...
/// <summary>
/// 랭킹 모드 씬에서 캐릭터 선택 버튼
/// RankingSceneButtonManager에 의해 관리됨
/// </summary>
public class CharacterSelectButton_RankingScene : MonoBehaviour
{
    ...
    [Header("생성할 오브젝트 프리팹")]
    [SerializeField] private GameObject deckSelectButtonPrefab; // 버튼 우측에 생성할 프리팹 (ex: 덱 선택 파업)

    [Header("RankingScene 버튼 매니저")]
    [SerializeField] public RankingSceneButtonManager manager;
    ...
    public void OnButtonClick() {
        ...
        if (manager == null) {
            Debug.LogError("[CharacterSelectButton] Manager를 찾을 수 없습니다.");
            return;
        }
